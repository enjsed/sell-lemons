...
local FX_SMOOTH; pcall_(function() FX_SMOOTH = Enum.Material.SmoothPlastic end)
local FX_LOW_MATERIAL = FX_SMOOTH or FX_PLASTIC
...
function FX.keep(inst)
    if not inst then return false end
    if inst == player.Character then return true end
    local ch = player.Character
    if ch then
        local ok, yes = pcall_(function() return inst:IsDescendantOf(ch) end)
        if ok and yes then return true end
    end
    return false
end
function FX.slim(inst)
    if not inst or FX.keep(inst) then return end
    local cn = tostring_(inst.ClassName)
    if cn == "ParticleEmitter" or cn == "Trail" or cn == "Beam"
       or cn == "Smoke" or cn == "Fire" or cn == "Sparkles" then
        FX.set(inst, "Enabled", false)
        pcall_(function() FX.set(inst, "Rate", 0) end)
    elseif cn == "Decal" or cn == "Texture" then
        FX.set(inst, "Transparency", 1)
    elseif cn == "PointLight" or cn == "SpotLight" or cn == "SurfaceLight" then
        FX.set(inst, "Enabled", false)
        pcall_(function() FX.set(inst, "Brightness", 0) end)
        pcall_(function() FX.set(inst, "Range", 0) end)
    elseif cn == "SurfaceAppearance" then
        FX.set(inst, "AlphaMode", 0)
    elseif cn == "SpecialMesh" then
        pcall_(function() FX.set(inst, "TextureId", "") end)
    elseif cn == "MeshPart" then
        if FX_LOW_MATERIAL then FX.set(inst, "Material", FX_LOW_MATERIAL) end
        FX.set(inst, "CastShadow", false)
        FX.set(inst, "Reflectance", 0)
        pcall_(function() FX.set(inst, "TextureID", "") end)
    else
        local isPart = false
        pcall_(function() isPart = inst:IsA("BasePart") end)
        if isPart then
            if FX_LOW_MATERIAL then FX.set(inst, "Material", FX_LOW_MATERIAL) end
            FX.set(inst, "CastShadow", false)
            FX.set(inst, "Reflectance", 0)
...
function FX.apply()
    ...
    task_spawn(function()
        pcall_(function()
            ...
            FX.set(lt, "Brightness", 1)
            pcall_(function() FX.set(lt, "EnvironmentDiffuseScale", 0) end)
            pcall_(function() FX.set(lt, "EnvironmentSpecularScale", 0) end)
            pcall_(function() FX.set(lt, "ShadowSoftness", 0) end)
                ...
                if cn == "BloomEffect" or cn == "BlurEffect" or cn == "SunRaysEffect"
                   or cn == "ColorCorrectionEffect" or cn == "DepthOfFieldEffect" or cn == "Atmosphere" then
                    if cn == "Atmosphere" then
                        FX.set(e, "Density", 0)
                        pcall_(function() FX.set(e, "Haze", 0) end)
                        pcall_(function() FX.set(e, "Glare", 0) end)
                    else
        ...
        pcall_(function()
            ...
            FX.conn = Workspace.DescendantAdded:Connect(function(inst)
                if FX.on and ScriptActive then
                    task_spawn(function()
                        task_wait(0.05)
                        if FX.on then FX.slim(inst) end
                    end)
                end
            end)
        end)
...
function FX.restore()
    ...
    FX.on = false
    pcall_(function() if FX.conn then FX.conn:Disconnect(); FX.conn = nil end end)
...
local RB = { mult = 2, rebirthMult = 2, gainPct = 100, lastPeek = 0, lastReb = 0, goSince = 0, peekEvery = 60, go = false, status = "off" }
...
if homesick then
    ...
    local window = homesick.createWindow("Sell Lemons", 500, 560)

    UIRef.win = window

    local tab1 = window:addTab("Main")

    local left = tab1:addSection("Automation", "Right")

    UIRef.t.AutoBuy = left:addToggle("autoBuy", "Auto Buy Buttons", false, function(val)
        autoBuyActive = val
    ...
    UIRef.t.SkipDecor = left:addToggle("skipDecor", "Skip Decoration Buttons", false, function(val)
        skipDecorActive = val
        S.saveState()
    ...
    UIRef.t.LemonFarm = left:addToggle("lemonFarm", "Fast Lemon Farm", false, function(val)
        lemonFarmActive = val
        S.saveState()
    ...
    pcall_(function()
        UIRef.t.AfkDelay = left:addSlider("afkDelay", "Start after idle", 1, 30, CFG.afkDelay or 6, function(val)
    ...
    UIRef.t.AutoStand = left:addToggle("autoStand", "Auto Upgrade Stands", false, function(val)
        autoStandActive = val
        S.saveState()
        print("[Hub] toggle AutoStand = " .. tostring_(val))
    end):addKeybind("3", "Toggle", true, function() end)

    UIRef.t.CashFarm = left:addToggle("cashFarm", "Auto Collect Cash Bags", true, function(val)
        cashFarmActive = val
        S.saveState()
    ...
    pcall_(function()
        RB.thBoost = 1
        RB.rebirthMult = RB.rebirthMult or 2
        RB.gainPct = (RB.rebirthMult - 1) * 100
        UIRef.rbMult = {}
        local function setRebirthMult(mult)
            RB.rebirthMult = mult
            RB.gainPct = (mult - 1) * 100
            for _, m in ipairs_({2, 3, 4, 5, 10}) do
                local cb = UIRef.rbMult[m]
                if cb and m ~= mult then pcall_(function() cb:SetValue(false) end) end
            end
            pcall_(function() if S.saveState then S.saveState() end end)
        end
        pcall_(function() left:addSeparator() end)
        for _, mult in ipairs_({2, 3, 4, 5, 10}) do
            UIRef.rbMult[mult] = left:addCheckbox("rebirthX" .. mult, "Rebirth at " .. mult .. "x", mult == RB.rebirthMult, function(val)
                if val then
                    setRebirthMult(mult)
                elseif RB.rebirthMult == mult then
                    pcall_(function() UIRef.rbMult[mult]:SetValue(true) end)
                end
            end)
        end
    end)

    local right = tab1:addSection("Extras", "Left")

    pcall_(function() window:setBadge("Sell Lemons v1  |  by enjsed") end)
    UIRef.t.AutoDeal = right:addToggle("autoDeal", "Auto Accept Deals", true, function(val)
        autoDealActive = val
        S.saveState()
    end)

    UIRef.t.AutoMini = right:addToggle("autoMini", "Auto Minigames", false, function(val)
        MG.active = val
    ...
    UIRef.t.AutoVine = right:addToggle("autoVine", "Auto Collect Cash Vine", CFG.autoVine == true, function(val)
        CFG.autoVine = val
        S.saveState()
        print("[Hub] toggle AutoVine = " .. tostring_(val))
    end)

    UIRef.t.CashVine = right:addToggle("cashVine", "Cash Vine Teleport", false, function(val)
        if val then
            CFG.vineGo = true
        else
            CFG.vineBack = true
    ...
    UIRef.t.KeyEsp = right:addToggle("keyEsp", "Key and Lever ESP", false, function(val)
        keyEspActive = val
        S.saveState()
    end)
    ...
    UIRef.t.FpsSave = right:addToggle("fpsSave", "Performance Mode", false, function(val)
...
local greyMemo = {}
pcall_(function() setmetatable(greyMemo, { __mode = "k" }) end)
local function isGreyedOut(v)
    local now = tick_()
    local m = greyMemo[v]
    if m and (now - m.t) < 0.20 then return m.v end
    ...
    local out = mabs(r - g) < 14 and mabs(g - b) < 14 and mabs(r - b) < 14 and mabs(r - 102) <= 22
    greyMemo[v] = { t = now, v = out }
    return out
end
...
local function getButtonsRealTime()
    ...
    local function scanTycoonButtons(tycoon)
    ...
    scanTycoonButtons(t)
    if #temp == 0 and (not t or (now - (_acache.refindT or 0)) > 1.0) then
        _acache.refindT = now
        local nt = findMyTycoon()
        if nt and nt ~= t then
            myTycoon = nt
            scanTycoonButtons(nt)
        end
    end
...
local _cashFolder
local _cashScan = { t = 0, list = nil }
local function getCashDropsFast()
    local now = tick_()
    if _cashScan.list and (now - _cashScan.t) < 0.20 then return _cashScan.list end
...
local LEMON_HITBOX_SIZE    = Vec3(72, 72, 72)

local LEMON_TP_WAIT      = 0.004
local LEMON_CAM_WAIT     = 0.004
local LEMON_CLICK_GAP    = 0.002
local LEMON_POST_WAIT    = 0
local LEMON_PASS_WAIT    = 0.015
...
local LEMON_VERIFY_WAIT       = 0.06
local LEMON_VERIFY_MAX_PASSES = 4
...
function LSM.collectVineAuto()
    if not CFG.autoVine then return false end
    if (tick_() - (LSM.vineCollectT or 0)) < 8 then return false end
    LSM.vineCollectT = tick_()
    local ok = false
    pcall_(function()
        local chr = player.Character
        local h = chr and chr:FindFirstChild("HumanoidRootPart")
        if not h then return end
        local ret = h.Position
        local vine = LSM.findVine()
        local pos = nil
        if vine then pos = _standPartPos(vine) end
        if not pos then pos = Vec3(39.7, -41.0, -77.5) end

        h.CFrame = CF(pos.X, pos.Y + 2, pos.Z)
        h.AssemblyLinearVelocity = Vec3(0, 0, 0)
        LSM.lastBot = tick_()
        task_wait(0.12)

        local prompt, clicker
        if vine then
            pcall_(function()
                for _, d in ipairs_(vine:GetDescendants()) do
                    local cn = tostring_(d.ClassName)
                    if cn == "ProximityPrompt" and not prompt then prompt = d end
                    if cn == "ClickDetector" and not clicker then clicker = d end
                    if prompt and clicker then break end
                end
            end)
        end

        if prompt and type(fireproximityprompt) == "function" then
            pcall_(fireproximityprompt, prompt)
        end
        if clicker and type(fireclickdetector) == "function" then
            pcall_(fireclickdetector, clicker)
        end
        for _ = 1, 8 do
            if not CFG.autoVine then break end
            pcall_(function() keypress(0x45) end)
            task_wait(0.04)
            pcall_(function() keyrelease(0x45) end)
            task_wait(0.08)
        end
        ...
        h.AssemblyLinearVelocity = Vec3(0, 0, 0)
        print("[Vine] auto collected -> 4h cooldown started")
        ok = true
    end)
    return ok
end
...
local function pollInput()
    ...
    local vineAutoReady = vReady == true
    if not vineAutoReady and CFG.vineT then
        vineAutoReady = (CFG.vineCd - (tick_() - CFG.vineT)) <= 0
    end
    if vineAutoReady and CFG.autoVine then CFG.vineAutoGo = true end
    ...
    if vReady == true then
        ...
        S.stLine(statusTx3, 3, CFG.autoVine and "cash vine  |  collecting" or "cash vine  |  READY")
...
_wrap("auto-cash-vine", function()
    while ScriptActive do
        if CFG.autoVine and CFG.vineAutoGo then
            CFG.vineAutoGo = false
            pcall_(LSM.collectVineAuto)
        end
        task_wait(0.5)
    end
end)
